    ...
    def step_monitor(self):
        ...
        if np.any(crossed):
            ...
            if (dt is not None) and (t_now is not None):
             ...
             # --- FILTRO x,y EN EL INSTANTE DEL CRUCE ---
            if (self.xlim is not None) or (self.ylim is not None):
                ...
                if not np.all(keep):
                    ...
                    # OJO: también debes filtrar los PIDs cruzados
                    #crossed_pids = set(np.asarray(list(crossed_pids))[keep].tolist())

                    #if self.include_dir and 'dir_sign' in locals():
                    #   dir_sign = dir_sign[keep]

                    if len(crossed_pids) > 0:
                        crossed_pids = set(np.asarray(list(crossed_pids))[keep].tolist())
